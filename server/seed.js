...
async function main() {
  ...
  try {
    ...
    if (existingUsers[0].n > 0 || existingPeople[0].n > 0 || existingShifts[0].n > 0) {
      console.log("Database already has data (users=%d, people=%d, shifts=%d). Skipping seed to avoid duplicating/overwriting real data.", existingUsers[0].n, existingPeople[0].n, existingShifts[0].n);
      console.log("Pass --force to seed anyway (this will NOT delete existing rows, only insert missing ones by id/email).");
    }
